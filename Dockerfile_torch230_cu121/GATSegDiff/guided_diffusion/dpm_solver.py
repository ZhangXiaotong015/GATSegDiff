...
class DPM_Solver:
    ...
    def noise_prediction_fn(self, x, t):
        ...
        if isinstance(out, tuple):
            return out[0], out[1] # pred_noise, seg
        return out
    ...
    def data_prediction_fn(self, x, t):
        ...
        """
        noise, seg = self.noise_prediction_fn(x, t)
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        x0 = (x - sigma_t * noise[:,0:1,:,:]) / alpha_t # only output the "mean"
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                t = timesteps[step]
                t_prev_list = [t]
                model_prev_list = [self.model_fn(x, t)[0]]
                if self.correcting_xt_fn is not None:
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(x, t)[0])
                # Compute the remaining values by `order`-th order multistep DPM-Solver.
                # indices = list(range(steps+1))[order:]
                for step in range(order, steps + 1):
                    ...
                    if step < steps:
                        model_prev_list[-1],seg = self.model_fn(x, t)
