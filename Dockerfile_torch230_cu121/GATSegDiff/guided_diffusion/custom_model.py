...
class CustomModel(nn.Module):
    def __init__(self, diff_model, dataloader, args, config, dataset, image_size):
        super().__init__()
        self.args = args
        self.config = config
        ...
        if dataset == "CELEBA":
            ...
            self.conv_out_la = DiagLaplace(nn.Sequential(self.conv_out, nn.Flatten(1, -1)), likelihood=self.likelihood, 
    ...
    def fit(self, train_loader, override=True):
        ...
        if self.dataset == "CELEBA":
            ...
            X, ct_25d, nodes_coord, edge_index, t, y = next(iter(train_loader))
            graph = {'nodes':nodes_coord.cuda(), 'edges':[item.cuda() for item in edge_index]}
            model_kwargs = {}
            model_kwargs.update(graph)

            with torch.no_grad():
                X_in = torch.cat((ct_25d, X), dim=1).to(self.conv_out_la._device)
                assert X_in.shape[1] == 4, f"Expected X_in.shape[1] to be 4, but got {X_in.shape[1]}"
                out = self.conv_out_la.model(self.feature_extractor(X_in, t.to(self.conv_out_la._device), **model_kwargs))
            self.conv_out_la.n_outputs = out.shape[-1]
            setattr(self.conv_out_la.model, 'output_size', self.conv_out_la.n_outputs)
            ...
            for X, ct_25d, nodes_coord, edge_index, t, y in train_loader:
                print(i)
                self.conv_out_la.model.zero_grad()
                ...
                X, t, y = X.to(self.conv_out_la._device), t.to(self.conv_out_la._device), y.to(self.conv_out_la._device)
                ct_25d, nodes_coord, edge_index = ct_25d.to(self.conv_out_la._device), nodes_coord.to(self.conv_out_la._device), [item.to(self.conv_out_la._device) for item in edge_index]
                graph = {'nodes':nodes_coord.cuda(), 'edges':[item.cuda() for item in edge_index]}
                model_kwargs = {}
                model_kwargs.update(graph)

                with torch.no_grad():
                    X_in = torch.cat((ct_25d, X), dim=1)
                    assert X_in.shape[1] == 4, f"Expected X_in.shape[1] to be 4, but got {X_in.shape[1]}"
                    X = self.feature_extractor(X_in, t, **model_kwargs)
                loss_batch, H_batch = self.conv_out_la._curv_closure(X, y, config, N)
                self.conv_out_la.loss += loss_batch
                self.conv_out_la.H += H_batch
                i+=1
    ...
    def forward(self, x, t, config, **model_kwargs):
        ...
        if self.dataset == "CELEBA":
            ...
            mean = torch.reshape(mean, (-1, 3, image_size, image_size))
            var = torch.reshape(var, (-1, 3, image_size, image_size))
            return (mean, var)
            ...
            # f_mean = torch.reshape(f_mean, (-1, 6, image_size, image_size))
            # f_var = torch.reshape(f_var, (-1, 6, image_size, image_size))
            f_mean = torch.reshape(f_mean, (-1, 2, image_size, image_size))
            f_var = torch.reshape(f_var, (-1, 2, image_size, image_size))

            # f_mean = torch.split(f_mean, 3, dim=1)[0]
            # f_var = torch.split(f_var, 3, dim=1)[0]
            f_mean = torch.split(f_mean, 1, dim=1)[0]
            f_var = torch.split(f_var, 1, dim=1)[0]
            return (f_mean, f_var)
    
    def accurate_forward(self, x, t, config, **model_kwargs):
        image_size = config.data.image_size
        ...
        if self.dataset == "CELEBA":
            ...
            with torch.no_grad():
                x = self.feature_extractor(x, t, **model_kwargs)
                acc_mean = self.copied_cov_out(x.float())

            # acc_mean = torch.reshape(acc_mean, (-1, 6, image_size, image_size))
            # acc_mean = torch.split(acc_mean, 3, dim=1)[0]       
            acc_mean = torch.reshape(acc_mean, (-1, 2, image_size, image_size))
            acc_mean = torch.split(acc_mean, 1, dim=1)[0]        
